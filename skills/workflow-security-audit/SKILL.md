...
```bash
# Pre-bundled binaries in .audit-bin/ (linux/amd64) avoid sandbox curl-pipe
# failures. Always try these first before any network install.
# See .audit-bin/README.md for how to update them.
...
- **Primary path:** pre-bundled linux/amd64 binaries in `.audit-bin/` are added to PATH first (step 0b). This is the reliable path — the GHA sandbox blocks `bash <(curl …)` pipe-installers and `~/.local/bin` is not on PATH, so runtime installs degrade to hand-rolled fallback. See `.audit-bin/README.md` for how to update these binaries when new versions are released.
- **Fallback:** if `.audit-bin/` binaries are absent or the wrong arch, `pipx install zizmor` and `pip install --user zizmor` hit PyPI — expected to work from standard GitHub-hosted runners, but not guaranteed in the Aeon sandbox.
