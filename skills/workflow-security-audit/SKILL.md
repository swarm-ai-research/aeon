...
# When auditing this skill, verify ACTIONLINT_VERSION is still current
# (https://github.com/rhysd/actionlint/releases) and bump if a new version is out.
# Also replace .audit-bin/actionlint (and actionlint.tar.gz) when bumping.
```
...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` (v1.25.2) and `.audit-bin/actionlint` (v1.7.12) are committed pre-built Linux x86-64 binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When a version pin in this file is bumped, replace the corresponding binary here too.
