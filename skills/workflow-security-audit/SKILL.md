...
```bash
# actionlint and zizmor are vendored in the repo root — add to PATH first so
# the network install steps below are skipped on the fast path.
...
- `actionlint` and `zizmor` binaries are vendored in the repo root (`./actionlint`, `./zizmor`) and added to `$PATH` at the top of step 0b, so the network install is skipped on normal runs. If the vendored binary is missing or stale, the pipx/pip and curl fallbacks below apply. To update, replace the binaries in the repo root and bump `ZIZMOR_VERSION` accordingly.
