...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed pre-built binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When `ZIZMOR_VERSION` is bumped, replace `.audit-bin/zizmor` to match. actionlint has no version pin here — the committed binary is the only version anchor, so note its version (e.g. in a commit message) when replacing it.
