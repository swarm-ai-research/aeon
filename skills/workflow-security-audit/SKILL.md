...
- **Pre-built fallback:** `.audit-bin/` in the repo root contains committed linux/amd64 binaries for both `actionlint` and `zizmor` (1.25.2). If all network installs fail, run `export PATH="$PWD/.audit-bin:$PATH"` before the scan steps — no network access required. See `.audit-bin/README.md` for update instructions.
