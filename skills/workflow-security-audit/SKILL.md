...
Pre-built binaries for the current pinned versions are committed to `.audit-bin/` (x86-64 Linux). Check there first to skip network downloads — faster and sandbox-safe:
...
# Update .audit-bin/zizmor when bumping the version.
...
- **Pre-built binaries in `.audit-bin/`** (x86-64 Linux) are the fastest path — no network calls, no PyPI. Step 0b checks them first. When upgrading tool versions, replace the binaries here and commit.
