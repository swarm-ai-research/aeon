...
```bash
# .audit-bin/ ships pre-committed binaries (zizmor 1.25.2, actionlint 1.7.12) that
# bypass sandbox restrictions — the GHA sandbox blocks both `bash <(curl …)` and
# execution of ~/.local/bin/* installed by pipx/pip. Add .audit-bin/ first so these
# take precedence when present; the pip/curl paths below remain as fallbacks for
# local runs where the directory may be absent.
...
# When auditing this skill, verify ZIZMOR_VERSION matches .audit-bin/zizmor and is
# still on the latest stable (https://github.com/zizmorcore/zizmor/releases); bump
# both ZIZMOR_VERSION and the binary together when upgrading.
# actionlint (Rhysd's syntax-level workflow linter — 1.7.12 in .audit-bin/)
...
- **`.audit-bin/` (preferred):** `zizmor` (1.25.2) and `actionlint` (1.7.12) are pre-committed as repo binaries and added to PATH in step 0b. This is the reliable path in GitHub Actions — the sandbox blocks `bash <(curl …)` installers and execution of `~/.local/bin/*` from pipx/pip. If `.audit-bin/` binaries are missing or stale, update them with a prefetch script before the Claude step.
- **PyPI fallback (local only):** `pipx install zizmor` / `pip install --user zizmor` work on local machines with outbound PyPI access. In GHA the sandbox may block these — `.audit-bin/` takes precedence via PATH ordering.
