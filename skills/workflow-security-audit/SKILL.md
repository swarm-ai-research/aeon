...
```bash
# Pre-committed binaries live in .audit-bin/ (committed to repo to avoid sandbox download blocks).
# Add them to PATH first so the command -v checks below find them without any network access.
...
If `.audit-bin/` binaries are present (committed to the repo) they are found immediately via the PATH prepend above — no network required. If the sandbox blocks the fallback download, use **WebFetch** to pull the install script, save it locally, and `bash` it. If both tools still fail to install, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
...
- **Pre-committed binaries:** `zizmor` and `actionlint` are committed under `.audit-bin/` so the skill runs without any network download in the GitHub Actions sandbox. Step 0b prepends `.audit-bin/` to PATH so they are found first. If the binaries become stale (wrong arch, wrong version), replace them by running the install commands locally and committing the new binaries.
