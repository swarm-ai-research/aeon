...
```bash
# Pre-bundled binaries live in .audit-bin/ (committed to repo) — prefer these
# over network installs to avoid sandbox outbound-network restrictions.
...
- **Pre-bundled binaries:** `.audit-bin/actionlint` and `.audit-bin/zizmor` are committed to the repo and added to `PATH` in step 0b. When present, they satisfy the `command -v` checks and the runtime installs below are skipped — no network required.
- If `.audit-bin/` binaries are absent or stale, `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners. If the sandbox blocks them, use **WebFetch** to retrieve the zizmor install script from `https://docs.zizmor.sh/install.sh` (or the release tarball from the `zizmorcore/zizmor` releases page) and run it locally.
