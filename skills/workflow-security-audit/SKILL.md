...
```bash
# Pre-fetched binaries land in .audit-bin/ (committed to the repo to avoid sandbox
# download failures — see memory note sandbox-blocks-piped-curl-installers).
# Check .audit-bin/ first before attempting network installs.
...
if [ -x ".audit-bin/actionlint" ]; then
  export PATH="$PWD/.audit-bin:$PATH"  # already set above if zizmor branch ran, but be explicit
...
If the sandbox blocks the download and `.audit-bin/` binaries are absent, use **WebFetch** to pull the install script, save it locally, and `bash` it. If both tools still fail to install, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
...
- **Pre-fetched binaries (primary path):** `zizmor` and `actionlint` executables are committed under `.audit-bin/` to avoid sandbox download failures (the GitHub Actions sandbox blocks `bash <(curl …)` installs and often blocks outbound PyPI calls — see memory note `sandbox-blocks-piped-curl-installers`). Step 0b adds `.audit-bin/` to `$PATH` when the binaries are present. Update `.audit-bin/` binaries when bumping `ZIZMOR_VERSION` or the actionlint release.
- **Fallback:** `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners with internet access. If both fail, use **WebFetch** to retrieve the zizmor install script from `https://docs.zizmor.sh/install.sh` (or the release tarball from the `zizmorcore/zizmor` releases page) and run it locally.
