...
```bash
# Pre-cached binaries live in .audit-bin/ (committed to the repo — see .audit-bin/README.md).
# Add to PATH first so network installs are skipped when the cache is present.
...
- **Pre-cached binaries** — `.audit-bin/zizmor` (v1.25.2) and `.audit-bin/actionlint` (v1.7.12) are committed to the repo and added to `PATH` before any network installs (see step 0b). On runners where pip/curl is blocked, these are the primary fallback. See `.audit-bin/README.md` for how to update them.
- `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners (outbound to PyPI is allowed), but if the sandbox blocks them the pre-cached binary is used; if that is also missing, use **WebFetch** to retrieve the zizmor install script from `https://docs.zizmor.sh/install.sh` (or the release tarball from the `zizmorcore/zizmor` releases page) and run it locally.
