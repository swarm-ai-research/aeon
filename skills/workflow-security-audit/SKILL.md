...
```bash
# .audit-bin/ holds pre-fetched zizmor and actionlint binaries committed to the repo.
# Prepend it first so the sandbox never needs to reach out to PyPI or curl.
[ -d ".audit-bin" ] && export PATH="$PWD/.audit-bin:$PATH"
...
- **Preferred (no network):** `.audit-bin/` contains pre-fetched `zizmor` and `actionlint` binaries committed to the repo. Step 0b prepends this directory to PATH, so both tools resolve without hitting PyPI or curl.
