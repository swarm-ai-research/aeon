...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` (v1.25.2) and `.audit-bin/actionlint` (v1.7.12) are committed pre-built binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When either version pin (`ZIZMOR_VERSION` / `ACTIONLINT_VERSION`) in this file is bumped, replace the corresponding binary in `.audit-bin/` too.
