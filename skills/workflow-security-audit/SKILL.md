...
# Pre-fetched binaries live in .audit-bin/ (committed to avoid sandbox download blocks).
# Check there first before attempting a network install.
...
- **Pre-fetched binaries:** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed to the repo so the skill works even when outbound network is blocked. Step 0b checks `.audit-bin/` before attempting any download. To update the binaries, download new releases and replace the files in `.audit-bin/` (also update `ZIZMOR_VERSION` above to match).
