...
```bash
# Pre-bundled binaries land in .audit-bin/ — try them first to avoid network calls.
# Fall back to network install only if the bundled copy is missing or non-executable.
export PATH=".audit-bin:$HOME/.local/bin:$PATH"
...
# Update the binary in .audit-bin/ to match when bumping (see .audit-bin/README.md).
...
- Pre-bundled `zizmor` and `actionlint` binaries live in `.audit-bin/` (committed to the repo). The install step adds `.audit-bin/` to `$PATH` first, so network downloads are only attempted when the bundled copy is missing. See `.audit-bin/README.md` for update instructions.
- If network installs are still needed: `pipx install zizmor` and `pip install --user zizmor` both hit PyPI. If the sandbox blocks them, use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` releases page and run the install locally.
