...
Pre-compiled binaries live in `.audit-bin/` (committed to the repo so the
sandbox doesn't need network access). Add that directory to PATH first, then
fall back to network installs only if the pre-committed binaries are absent.
If both paths fail, exit with `WORKFLOW_AUDIT_TOOL_FAIL`.

```bash
# Pre-committed binaries take priority — no network needed when they're present.
...
# Also update the binary in .audit-bin/ and the README there when bumping.
...
- **Pre-committed binaries (primary):** `.audit-bin/zizmor` and `.audit-bin/actionlint` are Linux x86-64 ELF binaries committed to the repo. Step 0b prepends `.audit-bin/` to PATH so they're found before any network install is attempted. See `.audit-bin/README.md` for how to update them.
- **pip/pipx fallback:** `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners, but if the sandbox blocks them use **WebFetch** to retrieve the zizmor release tarball from `zizmorcore/zizmor` releases and extract it to `.audit-bin/`.
