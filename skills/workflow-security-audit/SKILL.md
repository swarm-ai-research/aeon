...
```bash
# Prefer pre-downloaded binaries in .audit-bin/ (committed to avoid sandbox network blocks).
...
- Pre-downloaded binaries for zizmor and actionlint live in `.audit-bin/` and are added to `$PATH` first; if they are present and executable the network install steps are skipped entirely.
