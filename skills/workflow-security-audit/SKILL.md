...
Pre-built Linux x86-64 binaries are committed to `.audit-bin/` as an offline fallback for sandbox environments where network access is restricted. Check there first before attempting any download.
...
if ! command -v zizmor >/dev/null 2>&1; then
  # Prefer the pre-cached offline binary in .audit-bin/ (no network needed)
...
# When bumping, replace the .audit-bin/zizmor binary to match.
...
- **Pre-cached binaries:** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed to the repo as Linux x86-64 offline fallbacks. Step 0b checks them before attempting any network download. If the runner is a non-x86-64 or non-Linux platform, the binaries won't execute — fall through to the network path.
