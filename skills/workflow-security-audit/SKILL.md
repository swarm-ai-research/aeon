...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` (1.25.2) and `.audit-bin/actionlint` (1.7.12) are committed pre-built binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When a version pin in this file is bumped, replace the corresponding binary here too.
