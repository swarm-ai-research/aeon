...
Pre-committed binaries live in `.audit-bin/` as a sandbox workaround (the GitHub Actions sandbox blocks `bash <(curl …)` pipe installers). Always try `.audit-bin/` first before falling back to network installs.

```bash
# Prefer pre-committed binaries — no network required, sandbox-safe.
export PATH="$PWD/.audit-bin:$HOME/.local/bin:$PATH"

# zizmor (Trail of Bits, SARIF-capable GH Actions auditor)
# Pin to a specific version for reproducibility — bump .audit-bin/zizmor when upgrading.
...
- **Pre-committed binaries** (`zizmor` and `actionlint`) live in `.audit-bin/` — prepend it to `PATH` first. This bypasses both the `bash <(curl …)` pipe-install block and the `~/.local/bin` PATH issue observed on 2026-06-21.
- If `.audit-bin/` binaries are stale or missing, fall back to `pipx install zizmor` / `pip install --user zizmor`. Both hit PyPI, which is allowed from GitHub-hosted runners.
- If PyPI is also blocked, use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` GitHub Releases page and extract locally.
