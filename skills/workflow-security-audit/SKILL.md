...
- **Primary path:** `.audit-bin/` ships pre-built `zizmor` (1.25.2) and `actionlint` binaries committed to the repo. Step 0b puts `$PWD/.audit-bin` first on PATH, so both tools resolve from the repo without any network call. See `.audit-bin/README.md` for update instructions.
- **Fallback:** if `.audit-bin/` is absent or incompatible, step 0b falls back to `pipx install zizmor` (PyPI) and `bash <(curl ...)` for actionlint. If those also fail, use **WebFetch** to pull the release tarball from the `zizmorcore/zizmor` releases page and install locally.
