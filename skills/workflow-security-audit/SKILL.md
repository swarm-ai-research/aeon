...
# actionlint (Rhymond's syntax-level workflow linter) — version of the committed .audit-bin/actionlint binary.
...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed pre-built binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When bumping `ZIZMOR_VERSION` or `ACTIONLINT_VERSION` in this file, replace the corresponding binary in `.audit-bin/` and update the version table in `.audit-bin/README.md`.
