...
```bash
# Pre-seeded binaries in .audit-bin/ serve as the first-try fallback when the
# sandbox blocks network installs.  Add them to PATH before the install checks
# so `command -v` finds them immediately and the pip/curl paths become no-ops.
export PATH=".audit-bin:$HOME/.local/bin:$PWD:$PATH"
