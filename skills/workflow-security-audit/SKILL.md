...
# The committed .audit-bin/actionlint binary takes precedence; record its version in
# .audit-bin/README.md and keep this comment in sync when the binary is updated.
...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed pre-built Linux x86-64 binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. See `.audit-bin/README.md` for pinned versions and upgrade steps. When bumping `ZIZMOR_VERSION` or updating the actionlint binary, update `.audit-bin/README.md` to match.
