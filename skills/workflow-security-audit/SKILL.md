...
```bash
# Pre-built binaries (committed to .audit-bin/) take priority — avoids the
# sandbox curl-pipe / pipx blocks documented in memory/notes/sandbox-blocks-piped-curl-installers.md
[ -d "$PWD/.audit-bin" ] && export PATH="$PWD/.audit-bin:$PATH"
