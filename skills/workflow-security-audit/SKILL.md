...
if ! command -v zizmor >/dev/null 2>&1; then
  # Try pre-cached binary first (sandbox-safe; avoids blocked network installs)
...
if ! command -v actionlint >/dev/null 2>&1; then
  # Try pre-cached binary first (sandbox-safe; avoids blocked curl-pipe installs)
...
- **Pre-cached binaries:** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed to the repo as a sandbox-safe fallback. Step 0b checks these first before attempting any network install. If you upgrade either tool, update the binaries in `.audit-bin/` and bump `ZIZMOR_VERSION` above.
