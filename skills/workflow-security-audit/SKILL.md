...
- **Pre-cached binaries:** `.audit-bin/actionlint` and `.audit-bin/zizmor` are committed to the repo. Check for them before attempting any network install — `export PATH="$PWD/.audit-bin:$PATH"` is the fastest path and works fully offline. The install steps in § 0b already fall through to network only when the pre-cached binaries are absent.
