...
**Pre-cached binaries (first resort):** The repo ships zizmor and actionlint under `.audit-bin/` to bypass sandbox download restrictions. Check there before attempting any network install:

```bash
# Pre-cached binaries in .audit-bin/ take priority — avoids sandbox curl/pip failures.
# See .audit-bin/README.md for pinned versions and refresh instructions.
...
- **`.audit-bin/` pre-cached binaries (primary fallback):** The repo commits zizmor and actionlint under `.audit-bin/` — adding `$PWD/.audit-bin` to PATH is the fastest sandbox workaround and should be tried first (step 0b). See `.audit-bin/README.md` for pinned versions and instructions to refresh the cache when upgrading tools.
