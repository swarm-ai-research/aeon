...
- Pre-cached binaries live in `.audit-bin/` (see `.audit-bin/README.md`). If
  `pipx`/`pip`/`curl` are blocked, prepend that directory to `PATH` before the
  install block: `export PATH="$REPO_ROOT/.audit-bin:$PATH"`.
