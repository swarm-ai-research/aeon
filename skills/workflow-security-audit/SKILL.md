...
if ! command -v zizmor >/dev/null 2>&1; then
  # First: use the pre-cached binary in .audit-bin/ (avoids network in the sandbox).
...
if ! command -v actionlint >/dev/null 2>&1; then
  # First: use the pre-cached binary in .audit-bin/.
...
If the sandbox blocks network installs and `.audit-bin/` binaries are absent, use **WebFetch** to pull the install script, save it locally, and `bash` it. If all paths fail, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
...
- **Pre-cached binaries (preferred):** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed to the repo so the skill works without any network access. Step 0b checks for them first. If they are stale, update them by running the installers locally and committing the new binaries.
- `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners (outbound to PyPI is allowed), but the Aeon sandbox is known to block `bash <(curl …)` pipe-installs and `~/.local/bin` PATH resolution (observed 2026-06-21: `WORKFLOW_AUDIT_TOOL_DEGRADED`). Use `.audit-bin/` or **WebFetch** as fallbacks.
