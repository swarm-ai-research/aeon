...
# Pre-cached version: 1.7.12. When auditing this skill, check for a newer stable release
# at https://github.com/rhysd/actionlint/releases and update the binary in .audit-bin/ if needed.
...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` (v1.25.2) and `.audit-bin/actionlint` (v1.7.12) are committed pre-built binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When the version pin in this file is bumped, replace the binaries here too. **Invocation note:** direct `./` calls (e.g. `./.audit-bin/zizmor`) may be blocked by the Claude Code harness permission model; use `python3 subprocess.run(...)` as the invocation wrapper instead of bare shell redirection.
