...
Pre-compiled Linux x86-64 binaries ship in `.audit-bin/` (committed to the repo) to work around the GHA sandbox network restriction. Check there first before attempting any network install.
...
If `.audit-bin/` is missing and the sandbox blocks the download, use **WebFetch** to pull the install script, save it locally, and `bash` it. If both tools still fail to install, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
...
- Pre-compiled binaries for `zizmor` and `actionlint` (Linux x86-64) are committed to `.audit-bin/`. Step 0b checks there first, so no network install is needed in the normal GHA path.
- If `.audit-bin/` is stale or missing: `pipx install zizmor` and `pip install --user zizmor` both hit PyPI (outbound to PyPI is allowed from GitHub-hosted runners). If the sandbox blocks those too, use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` releases page and run it locally.
