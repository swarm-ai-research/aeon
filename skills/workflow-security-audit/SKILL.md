...
```bash
# Pre-cached binaries in .audit-bin/ (committed to the repo) take priority —
# avoids network round-trips on re-runs and works in sandboxed environments.
...
- **Pre-cached binaries:** `.audit-bin/` in the repo root contains committed `zizmor` and `actionlint` binaries. Step 0b prepends this directory to `$PATH` so they're used automatically without any network access. When upgrading tool versions, replace the binaries here and update `ZIZMOR_VERSION`.
- `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners (outbound to PyPI is allowed), but if the sandbox blocks them the `.audit-bin/` cache is the offline fallback. If the cache is also missing, use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` releases page and install locally.
