...
```bash
# Pre-bundled binaries land in .audit-bin/ when a prior run committed them.
# Try that directory first — it avoids all network installs inside the sandbox.
...
- **Pre-bundled binaries (`.audit-bin/`):** when a prior audit run succeeds it commits `zizmor` and `actionlint` into `.audit-bin/`. Step 0b prepends that directory to PATH so subsequent runs skip all network installs entirely — the fastest and most reliable path.
...
- The `bash <(curl ...)` pattern for actionlint is blocked inside the Claude Code sandbox per `memory/notes/sandbox-blocks-piped-curl-installers.md`; use the `.audit-bin/` binary or WebFetch the tarball and extract manually.
