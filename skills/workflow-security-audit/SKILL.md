...
```bash
# .audit-bin/ holds pre-cached binaries committed by previous runs — check it
# first to avoid unnecessary network calls inside the sandbox.
...
- **Pre-cached binaries:** `.audit-bin/` in the repo root holds `zizmor` and `actionlint` committed by the most recent audit run. Step 0b prepends `.audit-bin/` to `PATH`, so `command -v` finds them without any network call. If the cached version is too old (check `ZIZMOR_VERSION` above), the pip/curl fallback runs normally and the new binary overwrites the cache in the next commit.
