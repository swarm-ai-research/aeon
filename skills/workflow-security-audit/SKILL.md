...
Pre-built binaries are bundled in `.audit-bin/` (zizmor 1.25.2, actionlint 1.7.12) to avoid sandbox download failures — add that directory to PATH first. Fall back to network installs only if the bundled copy is absent. If both paths fail, exit with `WORKFLOW_AUDIT_TOOL_FAIL`.

```bash
# Prefer bundled binaries — sandbox blocks curl-pipe installs and ~/.local/bin exec
# (see memory/notes/sandbox-blocks-piped-curl-installers.md)
...
# When auditing this skill, verify ZIZMOR_VERSION matches the bundled binary in .audit-bin/
# and is still on the latest stable (https://github.com/zizmorcore/zizmor/releases);
# bump both together if a patch/minor is out.
...
If both tools still fail to install, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
...
- **Primary path:** `.audit-bin/` bundles pre-built `zizmor` (1.25.2) and `actionlint` (1.7.12) binaries tracked in the repo. Step 0b adds `.audit-bin/` to PATH first, so no network call is needed on a normal run.
- **Fallback:** if `.audit-bin/` is absent or stale, `pipx install zizmor` / `pip install --user zizmor` hit PyPI — allowed on GitHub-hosted runners. The `bash <(curl …)` actionlint installer is blocked by the sandbox (see `memory/notes/sandbox-blocks-piped-curl-installers.md`); if it fails and `.audit-bin/actionlint` is gone, the run degrades to hand-rolled checks only.
- **Upgrading bundled binaries:** download the new release, replace `.audit-bin/zizmor` and/or `.audit-bin/actionlint`, and bump `ZIZMOR_VERSION` in step 0b to match.
