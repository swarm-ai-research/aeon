...
**Prefer pre-built binaries first.** `.audit-bin/` ships committed binaries for both scanners, bypassing the sandbox's curl/pip block entirely (see Sandbox note). Add it to PATH before any network install attempt:

```bash
export PATH="$PWD/.audit-bin:$PATH"
```

Then fall back to network install only when the binary is missing (e.g. local dev runs):
...
- **`.audit-bin/` (primary path):** committed pre-built Linux amd64 binaries for `zizmor` and `actionlint` live here. Step 0b prepends this directory to PATH so sandbox runs never need network access for either tool. Update these binaries when bumping `ZIZMOR_VERSION` or the actionlint pin.
- **Network fallback (local dev / binary missing):** `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners (outbound to PyPI is allowed), but if the sandbox blocks them use **WebFetch** to retrieve the zizmor install script from `https://docs.zizmor.sh/install.sh` (or the release tarball from the `zizmorcore/zizmor` releases page) and run it locally.
