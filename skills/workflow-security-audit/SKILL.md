...
Pre-built binaries live in `.audit-bin/` (committed to the repo) and are preferred over network installs — they work in sandboxed runners with no outbound access. Fall back to network installs only when `.audit-bin/` is absent. If both paths fail, exit with `WORKFLOW_AUDIT_TOOL_FAIL`.
...
if ! command -v zizmor >/dev/null 2>&1; then
  # Prefer pre-fetched binary in .audit-bin/ (works offline / in sandboxed runners)
...
If the sandbox blocks the network download and `.audit-bin/` is absent, use **WebFetch** to pull the install script, save it locally, and `bash` it. If both tools still fail to install, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
...
- **Pre-built binaries** (`.audit-bin/zizmor`, `.audit-bin/actionlint`) are committed to the repo and used by default. No network access needed on runners that have the repo checked out.
