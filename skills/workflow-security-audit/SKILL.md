...
- **Pre-seeded binaries:** linux/amd64 builds of `zizmor` and `actionlint` live in `.audit-bin/`. Prepend `export PATH="$PWD/.audit-bin:$PATH"` at the top of step 0b so the `command -v` guards resolve immediately without any network call. This sidesteps the sandbox restrictions entirely.
