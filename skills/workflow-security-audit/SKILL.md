...
if ! command -v zizmor >/dev/null 2>&1; then
  # Fast path: pre-committed binary in .audit-bin/ (matches ZIZMOR_VERSION above).
...
# Also update the binary in .audit-bin/ to match (see .audit-bin/README.md).
# actionlint (rhysd/actionlint, syntax-level workflow linter)
if ! command -v actionlint >/dev/null 2>&1; then
  # Fast path: pre-committed binary in .audit-bin/.
...
If the sandbox blocks the live download and `.audit-bin/` is absent, use **WebFetch** to pull the install script, save it locally, and `bash` it. If both tools still fail to install, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
