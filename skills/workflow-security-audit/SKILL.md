...
Pre-built binaries live in `.audit-bin/` (committed to the repo) to avoid the GitHub Actions sandbox restriction that blocks `pip`/`pipx` installs and `bash <(curl …)` patterns. Always prefer those first.
...
If the sandbox blocks both the `.audit-bin/` path and the download fallbacks, use **WebFetch** to pull the install script, save it locally, and `bash` it. If both tools still fail to install, continue with the hand-rolled pattern checks in step 2 but mark the run as `WORKFLOW_AUDIT_TOOL_DEGRADED` in the footer.
...
- **Pre-built binaries** — `.audit-bin/actionlint` and `.audit-bin/zizmor` are committed to the repo and always tried first. They bypass pip/pipx/curl sandbox restrictions entirely.
- **PyPI fallback** — `pipx install zizmor` and `pip install --user zizmor` hit PyPI; expected to work from GitHub-hosted runners, but will fail in restricted sandboxes. If blocked, use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` releases page and install locally.
