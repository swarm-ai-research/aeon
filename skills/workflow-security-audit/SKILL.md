...
```bash
# .audit-bin/ holds pre-built binaries committed to the repo — check first to avoid
# sandbox download failures. Falls through to pipx/curl installs if absent.
...
- **Pre-cached binaries:** `.audit-bin/actionlint` and `.audit-bin/zizmor` are committed to the repo and prepended to `$PATH` in step 0b. When present they run entirely offline — no outbound network needed for the tool install phase.
