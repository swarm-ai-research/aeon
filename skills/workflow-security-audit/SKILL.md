...
```bash
# Pre-seeded binaries live in .audit-bin/ — add to PATH first so the
# command -v checks below find them before attempting live installs.
...
- **Pre-seeded binaries (check first):** `.audit-bin/` in the repo root contains committed `actionlint` and `zizmor` binaries (x86-64 Linux). Step 0b should prepend `.audit-bin/` to `PATH` before any live install — if present and executable, no network calls are needed. See `.audit-bin/README.md` for update instructions.
