...
# Pre-cached binary is v1.7.12; bump .audit-bin/actionlint when upgrading.
...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` (v1.25.2) and `.audit-bin/actionlint` (v1.7.12) are committed pre-built linux/amd64 binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When bumping a version pin, replace the matching binary in `.audit-bin/` and update the version comment in step 0b. See `.audit-bin/README.md` for update instructions.
