...
if ! command -v zizmor >/dev/null 2>&1; then
  # Pre-built binary in .audit-bin/ (committed to repo) is the fastest path and
  # avoids PyPI network calls that the GHA sandbox may block (see sandbox note below).
...
if ! command -v actionlint >/dev/null 2>&1; then
  # Pre-built binary in .audit-bin/ preferred; fall back to network install.
...
- **Pre-built binaries:** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed to the repo and are the preferred install path inside the GHA sandbox. Step 0b checks for them first. If they're present, no network call is needed.
- **Version pinning:** the binaries in `.audit-bin/` should match `ZIZMOR_VERSION` in step 0b. When bumping the version pin, replace both the variable and the `.audit-bin/zizmor` binary (download from `https://github.com/woodruffw/zizmor/releases`) and rebuild actionlint from `https://github.com/rhysd/actionlint/releases`.
