...
**Pre-bundled binaries (sandbox-safe, check first):** The repo ships static builds under `.audit-bin/` to avoid sandbox-blocked downloads. Add that directory to PATH before attempting any install.

```bash
# Pre-bundled static builds land here — prefer these over network installs.
# The sandbox blocks bash <(curl ...) and pip install, so this is the primary path.
...
- **`.audit-bin/` (primary):** Static `zizmor` and `actionlint` binaries are committed under `.audit-bin/` so the skill can run offline without hitting PyPI or GitHub download endpoints. Step 0b adds this directory to PATH first; the pip/curl fallbacks only fire if the pre-bundled binary is absent. To refresh binaries, run the audit skill from a network-capable environment and commit the updated executables.
