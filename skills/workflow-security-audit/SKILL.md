...
```bash
# Pre-cached binaries live in .audit-bin/ (committed to the repo).
# Add them to PATH first so the command-v checks below skip the network installs.
export PATH="$PWD/.audit-bin:$PWD/.audit-tools/bin:$PATH"
...
- **Pre-cached binaries:** zizmor and actionlint are committed to `.audit-bin/` (see `.audit-bin/README.md`). Step 0b adds `.audit-bin/` to `$PATH` before the `command -v` checks, so the network installs are skipped when the cache is present.
