...
# (https://github.com/rhysd/actionlint/releases) and bump if a new release is out.
# Also update the binary in .audit-bin/ when bumping the version pin.
```
...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` (v1.25.2) and `.audit-bin/actionlint` (v1.7.12) are committed pre-built binaries. Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When the version pins in this file are bumped, replace the binaries here too.
