...
Pre-seeded binaries live in `.audit-bin/` (committed to the repo so network downloads are
unnecessary on standard runs). Always prefer those over a fresh network install to avoid
sandbox blocking and to keep runs reproducible. Update `.audit-bin/` binaries when bumping
`ZIZMOR_VERSION` below (replace the committed file and update the tar.gz alongside it).
...
# Prefer the pre-committed binary in .audit-bin/ before falling back to network installs.
...
if [[ -x ".audit-bin/actionlint" ]]; then
  export PATH="$PWD/.audit-bin:$PATH"  # already set above if zizmor was found there
...
- **Pre-committed binaries:** `.audit-bin/actionlint` and `.audit-bin/zizmor` are committed to the repo and should resolve without any network access. Step 0b checks these first. If they are present and executable, no download is attempted.
- **Network fallback:** if `.audit-bin/` binaries are absent or non-executable, `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners (outbound to PyPI is allowed), but if the sandbox blocks them use **WebFetch** to retrieve the zizmor install script from `https://docs.zizmor.sh/install.sh` (or the release tarball from the `zizmorcore/zizmor` releases page) and run it locally.
