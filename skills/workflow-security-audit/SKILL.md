...
```bash
# Pre-bundled binaries in .audit-bin/ are the primary path — the GHA sandbox
# blocks both pipx/pip and the curl-pipe installer, so network installs fail.
# See .audit-bin/README.md for version info and how to update.
...
# Must match the binary committed to .audit-bin/zizmor.
...
- **Primary path:** `.audit-bin/zizmor` and `.audit-bin/actionlint` are pre-committed static binaries that resolve without any network call. Step 0b prepends `.audit-bin/` to `$PATH` so they are found first. See `.audit-bin/README.md` for version info and the update process.
- **Fallback:** `pipx install zizmor` and `pip install --user zizmor` hit PyPI — expected to work from GitHub-hosted runners, but if the sandbox blocks them use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` releases page and extract it locally. The curl-pipe installer for actionlint is also blocked in GHA; fall back to the same WebFetch-and-extract pattern.
