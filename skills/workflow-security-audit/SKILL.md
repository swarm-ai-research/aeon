...
```bash
# Pre-committed binaries land in .audit-bin/ — fastest path, no network needed.
# Always add this first so the pre-committed builds are preferred over any
...
# When bumping, also replace .audit-bin/zizmor with the new release binary.
...
- **Pre-committed binaries (primary):** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed to the repo and added to `$PATH` in step 0b — this is the reliable path because the sandbox blocks piped-curl installs and `~/.local/bin` may not be on PATH. See `.audit-bin/README.md` for version info and how to update them.
- **Runtime install (fallback):** `pipx install zizmor` and `pip install --user zizmor` both hit PyPI — expected to work from GitHub-hosted runners, but observed to fail inside the Claude Code sandbox. If both the pre-committed binary and the runtime install fail, use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` releases page and run it locally.
