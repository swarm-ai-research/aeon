...
**Pre-seeded binaries check first:** prior workflow runs may have committed pre-downloaded binaries to `.audit-bin/`. Add that directory to PATH before attempting network installs — if the binaries are there the `command -v` checks below will resolve them without any network fetch:
...
- **Pre-downloaded fallback (`.audit-bin/`):** workflow runs commit pre-downloaded `actionlint` and `zizmor` binaries into `.audit-bin/` when the tools are fetched successfully. Step 0b adds `.audit-bin/` to PATH first, so on subsequent runs the `command -v` checks hit the committed binaries and skip all network fetches — fully offline, no sandbox restriction can block it. If `.audit-bin/` is absent or stale, the pip/curl paths below apply.
