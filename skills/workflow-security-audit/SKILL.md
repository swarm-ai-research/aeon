...
**Pre-cached binaries (use these first — the GHA sandbox blocks curl-pipe installers):**

The repo ships pre-built binaries in `.audit-bin/` to avoid network installs in the sandbox.
Always prefer these over network downloads.

```bash
# Add pre-cached binaries to PATH before anything else
export PATH="$PWD/.audit-bin:$PATH"

# Verify the pre-cached tools are usable
zizmor --version >/dev/null 2>&1 || ZIZMOR_MISSING=1
actionlint --version >/dev/null 2>&1 || ACTIONLINT_MISSING=1
```

**Fallback — network install (only if `.audit-bin/` binaries are absent or non-executable):**
...
- **Pre-cached binaries in `.audit-bin/` are the primary path.** The GHA sandbox blocks `bash <(curl …)` installs and `~/.local/bin` isn't on PATH, so the fallback network install paths degrade to hand-rolled-only (observed 2026-06-21). Step 0b adds `.audit-bin/` to PATH first; network installs are the last resort.
- If `.audit-bin/` binaries are stale or missing, use **WebFetch** to retrieve the zizmor release tarball from the `zizmorcore/zizmor` releases page and extract it locally, then update `.audit-bin/zizmor`.
