...
- **`.audit-bin/` (primary):** `.audit-bin/zizmor` and `.audit-bin/actionlint` are committed pre-built binaries (Linux x86-64, for `ubuntu-latest` runners). Step 0b prioritises these over network installs to avoid sandbox-blocked PyPI/curl calls. When either version pin is bumped, replace the corresponding binary; see `.audit-bin/README.md` for platform notes and update steps.
