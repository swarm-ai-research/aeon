...
**Advisory mode (`advisory[owner/repo] == true`): skip this entire step.** Do not create or add labels — collect the would-be label set into the issue's advisory record (§6) instead.
...
- Never attempt label/comment/close writes on a repo where the §2 permission probe returned `false` — run advisory-only and record verdicts to `memory/triage-advisory/`.
