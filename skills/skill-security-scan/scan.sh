...
# Emit one formatted finding line per grep match for the given patterns.
# Usage: collect_matches <file> <pattern...>
collect_matches() {
  local file="$1"
  shift
  local pattern match line_num line_content
  for pattern in "$@"; do
    local matches
...
scan_file() {
  ...
  while IFS= read -r finding; do highs+=("$finding"); done \
    < <(collect_matches "$file" "${HIGH_PATTERNS[@]}")

  while IFS= read -r finding; do mediums+=("$finding"); done \
    < <(collect_matches "$file" "${MEDIUM_PATTERNS[@]}")

  while IFS= read -r finding; do lows+=("$finding"); done \
    < <(collect_matches "$file" "${LOW_PATTERNS[@]}")
