...
# Runs each pattern against file; results land in _scan_tmp.
_scan_tmp=()
collect_matches() {
  ...
  for pattern in "$@"; do
    ...
    while IFS= read -r match; do
      ...
      _scan_tmp+=("L${line_num}: ${line_content} [pattern: ${pattern}]")
    done <<< "$matches"
  done
}
...
scan_file() {
  ...
  collect_matches "$file" "${HIGH_PATTERNS[@]}";   highs=("${_scan_tmp[@]+"${_scan_tmp[@]}"}")
  collect_matches "$file" "${MEDIUM_PATTERNS[@]}"; mediums=("${_scan_tmp[@]+"${_scan_tmp[@]}"}")
  collect_matches "$file" "${LOW_PATTERNS[@]}";    lows=("${_scan_tmp[@]+"${_scan_tmp[@]}"}")
