...
# Print "Lnum: content [pattern: pat]" for every match across all given patterns.
grep_patterns() {
  local file="$1"; shift
  local patterns=("$@")
...
scan_file() {
  ...
  while IFS= read -r line; do highs+=("$line");   done < <(grep_patterns "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r line; do mediums+=("$line"); done < <(grep_patterns "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r line; do lows+=("$line");    done < <(grep_patterns "$file" "${LOW_PATTERNS[@]}")
