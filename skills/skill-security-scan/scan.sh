...
# Grep a file against a list of patterns; emit one "L<n>: <content> [pattern: <p>]" line per match.
collect_matches() {
  local file="$1"; shift
  for pattern in "$@"; do
    local matches
...
scan_file() {
  ...
  mapfile -t highs   < <(collect_matches "$file" "${HIGH_PATTERNS[@]}")
  mapfile -t mediums < <(collect_matches "$file" "${MEDIUM_PATTERNS[@]}")
  mapfile -t lows    < <(collect_matches "$file" "${LOW_PATTERNS[@]}")
