...
# Print one finding line per grep match: "L{n}: {content} [pattern: {pat}]"
...
scan_file() {
  ...
  while IFS= read -r finding; do highs+=("$finding"); done < <(collect_matches "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r finding; do mediums+=("$finding"); done < <(collect_matches "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r finding; do lows+=("$finding"); done < <(collect_matches "$file" "${LOW_PATTERNS[@]}")
