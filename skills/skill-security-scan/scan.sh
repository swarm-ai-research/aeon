...
# Print one finding per line for every pattern that matches in file.
grep_patterns() {
  local file="$1"
  shift
...
scan_file() {
  ...
  # Collect findings per severity
  while IFS= read -r f; do highs+=("$f"); done < <(grep_patterns "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r f; do mediums+=("$f"); done < <(grep_patterns "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r f; do lows+=("$f"); done < <(grep_patterns "$file" "${LOW_PATTERNS[@]}")
