...
scan_file() {
  ...
  while IFS= read -r item; do highs+=("$item");   done < <(_scan_patterns "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r item; do mediums+=("$item"); done < <(_scan_patterns "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r item; do lows+=("$item");    done < <(_scan_patterns "$file" "${LOW_PATTERNS[@]}")
