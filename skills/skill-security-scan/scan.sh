...
# Emit "L<num>: <content> [pattern: <p>]" lines for every pattern that matches.
...
scan_file() {
  ...
  while IFS= read -r m; do highs+=("$m"); done < <(collect_pattern_matches "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r m; do mediums+=("$m"); done < <(collect_pattern_matches "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r m; do lows+=("$m"); done < <(collect_pattern_matches "$file" "${LOW_PATTERNS[@]}")
