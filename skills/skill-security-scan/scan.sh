...
# Print formatted matches for a list of patterns against a file.
# Output format: "L<num>: <content_truncated> [pattern: <pat>]"
grep_patterns() {
  local file="$1"
  shift
  for pattern in "$@"; do
...
scan_file() {
  ...
  while IFS= read -r entry; do highs+=("$entry");   done < <(grep_patterns "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r entry; do mediums+=("$entry"); done < <(grep_patterns "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r entry; do lows+=("$entry");    done < <(grep_patterns "$file" "${LOW_PATTERNS[@]}")
