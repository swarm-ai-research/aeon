...
# Emit one finding line per stdout for each pattern that matches the file.
collect_matches() {
  local file="$1"; shift
  ...
  for pattern in "$@"; do
    ...
    while IFS= read -r match; do
      local line_num="${match%%:*}" line_content="${match#*:}"
...
scan_file() {
  ...
  while IFS= read -r line; do highs+=("$line");   done < <(collect_matches "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r line; do mediums+=("$line"); done < <(collect_matches "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r line; do lows+=("$line");    done < <(collect_matches "$file" "${LOW_PATTERNS[@]}")
