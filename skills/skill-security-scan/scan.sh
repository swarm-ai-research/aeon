...
# Print one finding line per match: "L<n>: <content> [pattern: <pat>]"
# Usage: _collect_matches FILE PATTERN...
_collect_matches() {
  local file="$1"; shift
...
scan_file() {
  ...
  while IFS= read -r finding; do highs+=("$finding"); done \
    < <(_collect_matches "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r finding; do mediums+=("$finding"); done \
    < <(_collect_matches "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r finding; do lows+=("$finding"); done \
    < <(_collect_matches "$file" "${LOW_PATTERNS[@]}")
