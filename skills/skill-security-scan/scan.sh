...
# Append grep matches for every pattern in <patterns_var> (array nameref) into
# <results_var> (array nameref), formatted as "L<n>: <content> [pattern: <p>]".
# Requires bash 4.3+ for local -n namerefs (satisfied by GitHub-hosted runners).
collect_matches() {
  local file="$1"
  local -n _patterns_ref="$2"
  local -n _results_ref="$3"
  local pattern matches match line_num line_content
  for pattern in "${_patterns_ref[@]}"; do
    ...
    if [[ -n "$matches" ]]; then
      while IFS= read -r match; do
        ...
        _results_ref+=("L${line_num}: ${line_content} [pattern: ${pattern}]")
      done <<< "$matches"
    fi
  done
}
...
scan_file() {
  ...
  collect_matches "$file" HIGH_PATTERNS highs
  collect_matches "$file" MEDIUM_PATTERNS mediums
  collect_matches "$file" LOW_PATTERNS lows
