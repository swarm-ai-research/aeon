...
# Emit "L<n>: <content> [pattern: <p>]" lines to stdout for each grep hit.
_collect_matches() {
  local file="$1"; shift
...
scan_file() {
  ...
  while IFS= read -r line; do highs+=("$line");   done < <(_collect_matches "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r line; do mediums+=("$line"); done < <(_collect_matches "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r line; do lows+=("$line");    done < <(_collect_matches "$file" "${LOW_PATTERNS[@]}")
