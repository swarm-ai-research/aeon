...
# Runs each pattern against file with grep; appends formatted results to _PATTERNS_OUT.
_PATTERNS_OUT=()
check_patterns() {
  local file="$1" pattern matches line_num line_content
  shift
  _PATTERNS_OUT=()
  for pattern in "$@"; do
    ...
    if [[ -n "$matches" ]]; then
      while IFS= read -r match; do
        line_num="${match%%:*}"
        line_content="${match#*:}"
        _PATTERNS_OUT+=("L${line_num}: ${line_content:0:120} [pattern: ${pattern}]")
      done <<< "$matches"
    fi
  done
}
...
scan_file() {
  ...
  check_patterns "$file" "${HIGH_PATTERNS[@]}"
  local highs=("${_PATTERNS_OUT[@]}")

  check_patterns "$file" "${MEDIUM_PATTERNS[@]}"
  local mediums=("${_PATTERNS_OUT[@]}")

  check_patterns "$file" "${LOW_PATTERNS[@]}"
  local lows=("${_PATTERNS_OUT[@]}")
