...
# Grep a list of patterns against a file; results land in global _MATCHED.
_MATCHED=()
grep_patterns() {
  local file="$1"; shift
  _MATCHED=()
  for pattern in "$@"; do
    ...
    if [[ -n "$matches" ]]; then
      while IFS= read -r match; do
        ...
        _MATCHED+=("L${line_num}: ${line_content} [pattern: ${pattern}]")
      done <<< "$matches"
    fi
  done
}
...
scan_file() {
  ...
  grep_patterns "$file" "${HIGH_PATTERNS[@]}"
  if [[ ${#_MATCHED[@]} -gt 0 ]]; then highs=("${_MATCHED[@]}"); fi

  grep_patterns "$file" "${MEDIUM_PATTERNS[@]}"
  if [[ ${#_MATCHED[@]} -gt 0 ]]; then mediums=("${_MATCHED[@]}"); fi

  grep_patterns "$file" "${LOW_PATTERNS[@]}"
  if [[ ${#_MATCHED[@]} -gt 0 ]]; then lows=("${_MATCHED[@]}"); fi
