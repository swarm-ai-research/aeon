...
# Print one "L<n>: <content> [pattern: <p>]" line per match to stdout.
collect_matches() {
  local file="$1" pattern match line_num line_content matches
  shift
  for pattern in "$@"; do
    ...
    if [[ -n "$matches" ]]; then
      while IFS= read -r match; do
        line_num="${match%%:*}"
        line_content="${match#*:}"
        printf '%s\n' "L${line_num}: ${line_content:0:120} [pattern: ${pattern}]"
      done <<< "$matches"
    fi
  done
}
...
scan_file() {
  ...
  while IFS= read -r item; do highs+=("$item"); done < <(collect_matches "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r item; do mediums+=("$item"); done < <(collect_matches "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r item; do lows+=("$item"); done < <(collect_matches "$file" "${LOW_PATTERNS[@]}")
