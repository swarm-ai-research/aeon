...
# Emit one "L<n>: <content> [pattern: <p>]" line per grep hit across all patterns.
collect_findings() {
  local file="$1"
  shift
  local pattern
...
scan_file() {
  ...
  while IFS= read -r entry; do highs+=("$entry");   done < <(collect_findings "$file" "${HIGH_PATTERNS[@]}")
  while IFS= read -r entry; do mediums+=("$entry"); done < <(collect_findings "$file" "${MEDIUM_PATTERNS[@]}")
  while IFS= read -r entry; do lows+=("$entry");    done < <(collect_findings "$file" "${LOW_PATTERNS[@]}")
