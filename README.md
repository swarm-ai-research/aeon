...
Aeon now also has a working prototype for running that fleet on [GitLawb](https://github.com/GitLawb), a decentralized, agent-first git substrate. The important part is not "Aeon on another host." The important part is that the prototype rebuilds the control layer GitHub normally provides: short-lived capabilities, gated renewal, portable self-evolution merge approval, and fleet observability.

The hardened prototype branch (`pr-4`) successfully ran four key scenarios:

- `demo.mjs` — dangerous capabilities expire quickly, escalation is rejected, and renewal/refusal acts as the off-switch
- `phase2.mjs` — GitLawb-hosted Aeon instances spawn, renew, fail renewal when they touch protected safety code, and lose power through expiry or blocklist
- `phase3.mjs` — self-evolution approval becomes a short-lived `pr/merge` capability bound to one exact diff rather than a GitHub branch-protection rule
- `phase5.mjs` — fleet events roll into an observability snapshot, while parallel and hierarchical sub-runs get scoped, self-expiring delegated authority

The core thesis is simple: Aeon can scale to a much larger autonomous fleet if authority becomes more temporary, more visible, and easier to withdraw as the fleet grows.
...
.audit-bin/              ← pre-built security audit binaries committed for sandbox use
  actionlint             ← GitHub Actions workflow linter (rhysd/actionlint)
  zizmor                 ← workflow security auditor (Trail of Bits); version pin in skills/workflow-security-audit/SKILL.md
scripts/
