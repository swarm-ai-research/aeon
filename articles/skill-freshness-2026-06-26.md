...
| vuln-scanner | `articles/vuln-scan-2026-06-20.md` | articles (weekly) | 153h | 192h | ✅ OK |

*All file ages for non-article deps are effectively 0h because this run occurs shortly after git checkout populates the working tree. The vuln-scan article age (153h) is computed from its filename date (2026-06-20); the next Saturday producer run is 2026-06-28, so it clears its 192h weekly threshold with 39h to spare.*

*Dedup: fingerprint identical to 2026-06-25 run (all-OK set, sha1 da39a3ee); no new flags — notification suppressed (FRESHNESS_NO_CHANGE).*
