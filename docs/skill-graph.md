...
> Auto-generated by `skill-graph` on 2026-08-02. Mode: `SKILL_GRAPH_NEW`.
...
flowchart LR
    ...
    subgraph other["Other & Uncategorized (18 / 38)"]
        other_anchor( )
    end
    crypto_anchor -->|"2"| other_anchor
    dev_anchor -->|"1"| other_anchor
...
Reactive triggers: `skill-repair` fires on `consecutive_failures >= 3` (`on: *`); `planner` fires on `consecutive_failures >= 2` (`on: *`).
...
flowchart LR
    ...
    pm-intel -..->|"topics/prediction-markets.md"| pm-manipulation
    pm-intel -..->|"topics/prediction-markets.md"| pm-pulse
...
flowchart LR
    ...
    pr-review["pr-review<br/><i>"0 9,18 * * *"  # reduced from every 6h to 2×/day to cut Actions burn — still covers morning + evening review cycles</i>"]
    ...
    vercel-projects -..->|"topics/repos.md"| external-feature
    vercel-projects -..->|"topics/vercel.md"| idea-pipeline
    vercel-projects -..->|"topics/repos.md"| repo-actions
    vercel-projects -..->|"topics/repos.md"| repo-scanner
...
flowchart LR
    ...
    market-context-refresh -..->|"topics/market-context.md"| idea-pipeline
    market-context-refresh -..->|"topics/market-context.md"| rwa-pulse
    market-context-refresh -..->|"topics/market-context.md"| topic-momentum
...
- `A --> B` — `depends_on` declared in `B`'s frontmatter
- `A -.-> B` — chain `consume:` edge (currently no active chains)
- `A -..-> B` — shared-state edge: `A` writes a `memory/topics/…` or `memory/state/…` file that `B` reads
- **Bold-bordered** nodes are enabled in `aeon.yml`; faded grey nodes are disabled
- Dashed `[external]` nodes are cross-category dependencies rendered inline for readability
- Click any node to open its `SKILL.md` on GitHub

## Summary

| Category | Total | Enabled | Disabled |
|---|---:|---:|---:|
| Research & Content | 24 | 1 | 23 |
| Dev & Code | 47 | 16 | 31 |
| Crypto & Markets | 24 | 0 | 24 |
| Social | 15 | 0 | 15 |
| Productivity & Meta | 26 | 9 | 17 |
| Other & Uncategorized | 38 | 18 | 20 |
| **Total** | **174** | **44** | **130** |
...
| `consume` (chains) | 0 |
| reactive | 2 |
| shared-state (derived) | 9 |

---

_skills parsed: 174 · depends_on: 5 · consume: 0 · reactive: 2 · shared-state derived: 9 · enabled: 44/174 · mode: SKILL_GRAPH_NEW_
