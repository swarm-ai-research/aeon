...
flowchart LR
  ...
  style ext_research fill:none,stroke:#ddd,stroke-dasharray:3 3
...
flowchart LR
  ...
  style ext_dev fill:none,stroke:#ddd,stroke-dasharray:3 3
...
flowchart LR
  ...
  style ext_crypto fill:none,stroke:#ddd,stroke-dasharray:3 3
...
flowchart LR
  ...
  style ext_productivity fill:none,stroke:#ddd,stroke-dasharray:3 3
...
> Note: every skill also writes `memory/cron-state.json` (consumed by `heartbeat` / `skill-health` / `skill-repair`) — collapsed to the self-healing loop diagram above. `agi-tracker` is enabled in `aeon.yml` but has no `SKILL.md` (see [[agi-tracker-missing-skill-md-dispatches-no-op]]) and is excluded from all counts.
