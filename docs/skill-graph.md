...
_Auto-generated by `skill-graph` on 2026-07-05 · mode: `SKILL_GRAPH_NEW`_

**Verdict:** NEW_SKILLS: initialized (172 skills mapped)

---

## Overview — five categories

Edges between boxes count all cross-category `depends_on` and shared-state links. Click into a category below for the full node list.

```mermaid
flowchart LR
  subgraph research["Research & Content<br/>32 skills · 5 enabled"]
  end
  subgraph dev["Dev & Code<br/>75 skills · 29 enabled"]
  end
  subgraph crypto["Crypto & Markets<br/>24 skills · 0 enabled"]
  end
  subgraph social["Social<br/>15 skills · 0 enabled"]
  end
  subgraph productivity["Productivity & Meta<br/>26 skills · 9 enabled"]
  end
  crypto -->|1| dev
  crypto -->|2| research
  research -->|2| crypto
...
The reliability quintet — `heartbeat`, `skill-health`, `skill-evals`, `skill-repair`, `self-improve` — coordinates through `memory/cron-state.json`. Every enabled skill writes to `cron-state.json` on run (success/failure counters). This is shown once here and **collapsed** in the per-category diagrams as a legend note, not repeated as N edges.
...
### Research & Content — 32 skills (5 enabled)
...
flowchart LR
  ...
  trigger_planner[/"trigger: *<br/>consecutive_failures >= 2"/]
...
### Crypto & Markets — 24 skills (0 enabled)
...
### Productivity & Meta — 26 skills (9 enabled)
...
flowchart LR
  ...
  trigger_skill-repair[/"trigger: *<br/>consecutive_failures >= 3"/]
  trigger_skill-repair -.-> skill-repair
  classDef enabled fill:#fff,stroke:#000,stroke-width:2px,color:#000
  classDef disabled fill:#f5f5f5,stroke:#bbb,color:#888
  classDef external fill:none,stroke:#bbb,stroke-dasharray:3 3,color:#888
...
- `A --> B` — explicit `depends_on` from A's SKILL.md frontmatter
- `A -.-> B` — chain `consume:` edge (from `aeon.yml` `chains:`)
- `A -..-> B` — derived shared-state edge (A writes a `memory/topics/*` or `memory/state/*` file that B reads)
- `trigger -.-> B` — reactive wake-up (from `aeon.yml` `reactive:`)
- **Bold black border** — `enabled: true` in `aeon.yml`; schedule shown under node
- Faded grey — disabled
- Dashed ghost node — external cross-category skill referenced by an edge in this diagram (click through for full context)
- Node click opens the corresponding `SKILL.md`
- `memory/cron-state.json` is a shared bus every skill writes on run; collapsed into the self-healing loop above rather than fanned out as N edges.

## Summary

| Category | Skills | Enabled |
|---|---:|---:|
| Research & Content | 32 | 5 |
| Dev & Code | 75 | 29 |
| Crypto & Markets | 24 | 0 |
| Social | 15 | 0 |
...
| `consume` (chains) | 0 |
| reactive | 2 |
| shared-state (derived) | 26 |

_Source-status: skills parsed: 172 · depends_on: 5 · consume: 0 · reactive: 2 · shared-state derived: 26 · enabled: 43/172 · mode: SKILL_GRAPH_NEW_
