...
> Auto-generated by `skill-graph` on 2026-07-19. Mode: `SKILL_GRAPH_NEW`.
...
flowchart LR
    ...
    subgraph crypto["Crypto & Markets (0 / 19)"]
        crypto_anchor( )
    end
    subgraph social["Social (0 / 12)"]
        social_anchor( )
    end
    subgraph productivity["Productivity & Meta (11 / 28)"]
        productivity_anchor( )
    end
    subgraph other["Other & Uncategorized (21 / 57)"]
        other_anchor( )
    end
    dev_anchor -->|"4"| other_anchor
    other_anchor -->|"1"| crypto_anchor
...
Reactive triggers: `skill-repair` fires on `consecutive_failures >= 3` for any skill (*). `planner` fires on `consecutive_failures >= 2` for any skill (*).
...
- `A --> B` : `A` declares `depends_on: [B]` in its frontmatter
- `A -.-> B` : `B` consumes `A` output via a chain (`consume:`)
- `A -..-> B` : shared-state edge — `A` writes a memory/topics or memory/state path that `B` reads
- **Enabled** skills: bold black border, schedule annotation shown
- **Disabled** skills: faded grey
- Faded external nodes (`external` class) are cross-category dependencies rendered as ghosts inside a category diagram
- Click any node on github.com to jump to its `SKILL.md`
...
| `consume` (chain) | 0 |
| `reactive` triggers | 2 |
| shared-state (derived) | 23 |

_skills parsed: 172 · depends_on: 5 · consume: 0 · reactive: 2 · shared-state derived: 23 · enabled: 43/172 · mode: SKILL_GRAPH_NEW_
