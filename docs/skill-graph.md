...
> Auto-generated by `skill-graph` on 2026-08-23. Mode: `SKILL_GRAPH_NEW`.
...
flowchart LR
    ...
    dev --> |1| other
    other --> |1| dev
    research --> |16| productivity
    research --> |8| social
    classDef cat fill:#fafafa,stroke:#333,stroke-width:2px
    class research cat
    class dev cat
    class crypto cat
    class social cat
    class productivity cat
    class other cat
```

## Self-healing loop

Every skill writes `memory/cron-state.json`; the callout below shows how the reliability chain reads it. This one shared-state fan-in is **collapsed into a single edge per skill** — do not confuse with per-skill dependencies.
...
flowchart LR
    ...
    classDef state fill:#eef,stroke:#557,color:#334,stroke-dasharray:3 3
...
Reactive wake-ups (from `aeon.yml`):

- **planner** wakes on `*` when `consecutive_failures >= 2`
- **skill-repair** wakes on `*` when `consecutive_failures >= 3`



## Per-category diagrams

Each category renders its own subgraph. Cross-category references appear as faded ghost nodes on the right; click any node to jump to its `SKILL.md`.

### Research & Content (0 / 19 enabled · 0 intra edges · 24 cross-cat refs)

```mermaid
flowchart LR
    subgraph research_core["Research & Content"]
        article[article]
    ...
    subgraph externals["cross-category refs"]
    ...
    class rss-feed external
    class syndicate-article external
    class update-gallery external
...
### Dev & Code (10 / 36 enabled · 6 intra edges · 2 cross-cat refs)

```mermaid
flowchart LR
    subgraph dev_core["Dev & Code"]
    ...
    repo-actions -..-> external-feature
    vercel-projects -..-> external-feature
    stale-content-pr-sweeper -..-> notegraph
    ...
    class notegraph external
    class vuln-tracker external
...
### Crypto & Markets (0 / 18 enabled · 0 intra edges · 3 cross-cat refs)

```mermaid
flowchart LR
    subgraph crypto_core["Crypto & Markets"]
...
### Social (0 / 12 enabled · 0 intra edges · 8 cross-cat refs)

```mermaid
flowchart LR
    subgraph social_core["Social"]
    ...
    subgraph externals["cross-category refs"]
        ...
        channel-recap[channel-recap]:::external
        competitor-launch-radar[competitor-launch-radar]:::external
        deep-research[deep-research]:::external
        last30[last30]:::external
        paper-digest[paper-digest]:::external
        research-brief[research-brief]:::external
        technical-explainer[technical-explainer]:::external
    end
    technical-explainer -.-> syndicate-article
    competitor-launch-radar -.-> syndicate-article
    paper-digest -.-> syndicate-article
    ...
    class channel-recap external
    class competitor-launch-radar external
    class deep-research external
    class last30 external
    class paper-digest external
    class research-brief external
    class technical-explainer external
...
### Productivity & Meta (9 / 26 enabled · 2 intra edges · 16 cross-cat refs)

```mermaid
flowchart LR
    subgraph productivity_core["Productivity & Meta"]
...
### Other & Uncategorized (24 / 61 enabled · 7 intra edges · 5 cross-cat refs)

```mermaid
flowchart LR
    subgraph other_core["Other & Uncategorized"]
    ...
    class market-context-refresh external
    class stale-content-pr-sweeper external
    class vuln-scanner external
...
- `A --> B` — `depends_on` (explicit, from SKILL.md frontmatter)
- `A -.-> B` — `consume` (chain step) **or** content-pipeline (research → syndicate/rss/gallery, derived)
- `A -..-> B` — reactive trigger **or** shared-state (writer → reader through `memory/topics/` or `memory/state/`, derived)
- **Enabled** nodes: bold border, schedule shown in label
- **Disabled** nodes: faded grey fill
- **Ghost** nodes (dashed border): the same skill in another category — click to jump
- `memory/cron-state.json` is written by nearly every skill; this map collapses that fan-in to the self-healing callout instead of drawing 18× edges

## Summary

| Category | Total | Enabled | Disabled |
|---|---:|---:|---:|
| Research & Content | 19 | 0 | 19 |
| Dev & Code | 36 | 10 | 26 |
| Crypto & Markets | 18 | 0 | 18 |
| Social | 12 | 0 | 12 |
| Productivity & Meta | 26 | 9 | 17 |
| Other & Uncategorized | 61 | 24 | 37 |
| **All** | **172** | **43** | **129** |

| Edge type | Count | Mermaid syntax |
|---|---:|---|
| depends_on | 5 | `A --> B` |
| consume (chain) | 0 | `A -.-> B` |
| reactive | 0 | `A -..-> B` |
| shared_state (derived) | 15 | `A -..-> B` |
| content-pipeline (derived) | 24 | `A -.-> B` |

## Source status

`skills parsed: 172 · depends_on: 5 · consume: 0 · reactive: 0 · shared-state derived: 15 · content-pipeline derived: 24 · enabled: 43/172 · mode: SKILL_GRAPH_NEW`
