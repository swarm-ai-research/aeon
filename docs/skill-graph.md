...
> Auto-generated by `skill-graph` on 2026-07-12. Mode: `SKILL_GRAPH_NEW`.
...
```mermaid
flowchart LR
    subgraph research["Research & Content (0 / 25)"]
        research_anchor( )
    end
    subgraph dev["Dev & Code (11 / 41)"]
        dev_anchor( )
    end
    subgraph crypto["Crypto & Markets (1 / 25)"]
        crypto_anchor( )
    end
    subgraph social["Social (0 / 15)"]
        social_anchor( )
    end
    subgraph productivity["Productivity & Meta (29 / 66)"]
        productivity_anchor( )
    end
    crypto_anchor -->|"3"| productivity_anchor
    crypto_anchor -->|"1"| research_anchor
    crypto_anchor -->|"1"| social_anchor
    dev_anchor -->|"4"| productivity_anchor
    productivity_anchor -->|"1"| dev_anchor
    research_anchor -->|"1"| crypto_anchor
    research_anchor -->|"16"| productivity_anchor
    research_anchor -->|"8"| social_anchor
...
flowchart LR
    ...
    planner -.->|"reactive: consec_fail >= 2"| skill-repair
    heartbeat -.-> cron
    skill-health -.-> cron
    skill-evals -.-> cron
    skill-repair -.-> cron
...
flowchart LR
    ...
    subgraph external_research["cross-category refs"]
    ext_research_pm-manipulation["pm-manipulation<br/><small>(crypto)</small>"]:::external
    ext_research_rss-feed["rss-feed<br/><small>(productivity)</small>"]:::external
    ext_research_syndicate-article["syndicate-article<br/><small>(social)</small>"]:::external
    ext_research_update-gallery["update-gallery<br/><small>(productivity)</small>"]:::external
    end
    pm-intel -..-> ext_research_pm-manipulation
    ai-framework-watch --> ext_research_rss-feed
    ai-framework-watch --> ext_research_syndicate-article
    ai-framework-watch --> ext_research_update-gallery
    competitor-launch-radar --> ext_research_rss-feed
    competitor-launch-radar --> ext_research_syndicate-article
    competitor-launch-radar --> ext_research_update-gallery
    content-engine --> ext_research_rss-feed
    content-engine --> ext_research_syndicate-article
    content-engine --> ext_research_update-gallery
    deep-research --> ext_research_rss-feed
    deep-research --> ext_research_syndicate-article
    deep-research --> ext_research_update-gallery
    ecosystem-pulse --> ext_research_rss-feed
    ecosystem-pulse --> ext_research_syndicate-article
    ecosystem-pulse --> ext_research_update-gallery
    last30 --> ext_research_rss-feed
    last30 --> ext_research_syndicate-article
    last30 --> ext_research_update-gallery
    paper-digest --> ext_research_rss-feed
    paper-digest --> ext_research_syndicate-article
    paper-digest --> ext_research_update-gallery
    research-brief --> ext_research_rss-feed
    research-brief --> ext_research_syndicate-article
    research-brief --> ext_research_update-gallery
...
flowchart LR
    ...
    subgraph external_dev["cross-category refs"]
    ext_dev_contributor-spotlight["contributor-spotlight<br/><small>(productivity)</small>"]:::external
    ext_dev_fleet-state["fleet-state<br/><small>(productivity)</small>"]:::external
    ext_dev_idea-pipeline["idea-pipeline<br/><small>(productivity)</small>"]:::external
    end
    ...
    builder-map -..-> ext_dev_idea-pipeline
    deploy-prototype -..-> ext_dev_idea-pipeline
    fork-cohort -..-> ext_dev_contributor-spotlight
    fork-cohort -..-> ext_dev_fleet-state
...
flowchart LR
    ...
    subgraph external_crypto["cross-category refs"]
    ext_crypto_idea-pipeline["idea-pipeline<br/><small>(productivity)</small>"]:::external
    ext_crypto_rss-feed["rss-feed<br/><small>(productivity)</small>"]:::external
    ext_crypto_syndicate-article["syndicate-article<br/><small>(social)</small>"]:::external
    ext_crypto_topic-momentum["topic-momentum<br/><small>(research)</small>"]:::external
    ext_crypto_update-gallery["update-gallery<br/><small>(productivity)</small>"]:::external
    end
    market-context-refresh -..-> rwa-pulse
    pm-pulse -..-> pm-manipulation
    market-context-refresh -..-> ext_crypto_idea-pipeline
    market-context-refresh -..-> ext_crypto_topic-momentum
    pm-manipulation --> ext_crypto_rss-feed
    pm-manipulation --> ext_crypto_syndicate-article
    pm-manipulation --> ext_crypto_update-gallery
...
flowchart LR
    ...
    subgraph external_productivity["cross-category refs"]
    ext_productivity_vuln-scanner["vuln-scanner<br/><small>(dev)</small>"]:::external
    end
    ...
    vuln-tracker --> ext_productivity_vuln-scanner
...
| Edge | Meaning |
|------|---------|
| `-->` | `depends_on:` in SKILL.md frontmatter, or research→content-pipeline auto-edge |
| `-.->` | `consume:` in a `chains:` step in aeon.yml |
| `-..->` | derived shared-state (writer → reader, same `memory/topics/*` or `memory/state/*` file) |

| Node style | Meaning |
|------------|---------|
| bold border | `enabled: true` in aeon.yml (schedule shown under name) |
| faded grey | `enabled: false` |
| dashed grey | cross-category ghost reference (drawn in the source category, real node lives elsewhere) |

Every node is clickable and links to the underlying `SKILL.md` on github.com.

_Cron-state fan-in collapsed: every skill writes `memory/cron-state.json` on every run. Rather than 172 dashed edges into the self-healing loop, the fan-in is documented once in the callout above._
...
| Enabled | 41 |
| Disabled | 131 |
| Category · Research & Content | 25 (0 enabled) |
| Category · Dev & Code | 41 (11 enabled) |
| Category · Crypto & Markets | 25 (1 enabled) |
| Category · Social | 15 (0 enabled) |
| Category · Productivity & Meta | 66 (29 enabled) |
| Edges · `depends_on` | 5 |
| Edges · `consume` | 0 |
| Edges · reactive triggers | 2 |
| Edges · derived shared-state | 22 |
| Edges · content pipeline (auto) | 27 |

---

skills parsed: 172 · depends_on: 5 · consume: 0 · reactive: 2 · shared-state derived: 22 · enabled: 41/172 · mode: SKILL_GRAPH_NEW
