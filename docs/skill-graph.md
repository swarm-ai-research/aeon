...
*Auto-generated by `skill-graph` on 2026-08-30. Mode: `SKILL_GRAPH_NEW`.*
...
Categories with cross-category dependency counts. Solid arrow labels show total edges (depends\_on + reactive + shared-state) between categories.

```mermaid
flowchart LR
  research["Research & Content<br/>24 skills"]
  dev["Dev & Code<br/>41 skills"]
  crypto["Crypto & Markets<br/>24 skills"]
  social["Social<br/>15 skills"]
  productivity["Productivity & Meta<br/>61 skills"]
  other["Other<br/>7 skills"]
  productivity -- 1 --> dev
  dev -- 5 --> productivity
...
The reliability sub-fleet: heartbeat detects, skill-health scores, skill-evals grades outputs, skill-repair fixes, self-improve reflects. Every enabled skill writes `memory/cron-state.json`; that shared state file drives the reactive triggers for `planner` (at 2 consecutive failures) and `skill-repair` (at 3).

```mermaid
flowchart LR
  heartbeat["heartbeat · 0 8 * * *"]
  skill-health["skill-health · 0 18 * * *"]
  skill-evals["skill-evals · 0 9 * * 0"]
  skill-repair["skill-repair · reactive"]
  self-improve["self-improve"]
  cron_state[("memory/cron-state.json")]
  ...
  cron_state -. "consecutive_failures>=3" .-> skill-repair
  cron_state -. "consecutive_failures>=2" .-> planner
  planner["planner · 30 6 * * *"]
  ...
  class heartbeat,skill-health,skill-evals,skill-repair,planner enabled
...
### Research & Content — 24 skills

```mermaid
flowchart LR
  subgraph research["research (24)"]
    agent-displacement["agent-displacement"]
    ai-framework-watch["ai-framework-watch · 30 8 * * 1"]
    article["article"]
  ...
  subgraph external["cross-category"]
...
flowchart LR
  subgraph dev["dev (41)"]
    ...
    vuln-scanner["vuln-scanner · 0 16 * * 6"]
    workflow-security-audit["workflow-security-audit · 0 16 * * 0"]
  ...
  classDef disabled fill:#f5f5f5,stroke:#bbb,color:#888
  classDef external fill:none,stroke:#bbb,stroke-dasharray:3 3,color:#888
  class changelog,code-health,fleet-control,github-monitor,gitlawb-fleet-metrics,issue-triage,pr-review,pr-triage,repo-revive,vuln-scanner,workflow-security-audit enabled
  class auto-merge,auto-workflow,autoresearch,builder-map,create-skill,deploy-prototype,external-feature,feature,fork-cohort,fork-first-run-alert,fork-fleet,fork-skill-gap,github-issues,github-releases,github-trending,gitlawb-fleet,project-lens,push-recap,repo-actions,repo-article,repo-pulse,repo-scanner,search-skill,skill-enabler,skill-security-scan,smithery-manifest,spawn-instance,star-milestone,vercel-projects,x402-monitor disabled
  class idea-pipeline,contributor-spotlight,fleet-state,vuln-tracker external
...
flowchart LR
  ...
  market-context-refresh -..->|"market-context.md"| rwa-pulse
  ...
  class idea-pipeline,topic-momentum,pm-intel external
...
### Productivity & Meta — 61 skills

```mermaid
flowchart LR
  subgraph productivity["productivity (61)"]
    action-converter["action-converter"]
    autoresearch-goal["autoresearch-goal"]
    batch-health["batch-health · 0 8 * * *"]
    config-validator["config-validator · 0 7 * * 0"]
    contributor-spotlight["contributor-spotlight"]
    cost-report["cost-report · 0 7 * * 1"]
    ...
    goal-tracker["goal-tracker · 0 18 * * *"]
    heartbeat["heartbeat · 0 8 * * *"]
    idea-capture["idea-capture"]
    idea-pipeline["idea-pipeline"]
    idea-validator["idea-validator"]
    janitor["janitor · 30 5 * * 0"]
    launch-radar["launch-radar"]
    memory-flush["memory-flush · 0 6 2/2 * *"]
    memory-structural-dedupe["memory-structural-dedupe · 10 6 2/2 * *"]
    milestone-tracker["milestone-tracker · 0 12 * * 1"]
    morning-brief["morning-brief"]
    note-taking["note-taking"]
    notegraph["notegraph · 0 5 * * *"]
    onboard["onboard"]
    operator-scorecard["operator-scorecard"]
    planner["planner · 30 6 * * *"]
    pr-tracker["pr-tracker · 0 10 * * *"]
    pvr-triage-monitor["pvr-triage-monitor"]
    pvr-watchlist["pvr-watchlist"]
    reflect["reflect · 0 18 * * *"]
    reg-monitor["reg-monitor"]
    rss-feed["rss-feed"]
    run-frequency-guard["run-frequency-guard · 0 23 * * *"]
    self-improve["self-improve"]
    self-review["self-review · 30 18 * * 0"]
    signal-verdict["signal-verdict"]
    skill-analytics["skill-analytics · 30 18 * * 3"]
    skill-evals["skill-evals · 0 9 * * 0"]
    skill-freshness["skill-freshness · 0 8 * * *"]
    skill-graph["skill-graph · 0 17 * * 0"]
    skill-health["skill-health · 0 18 * * *"]
    skill-leaderboard["skill-leaderboard"]
    skill-repair["skill-repair · reactive"]
    skill-update-check["skill-update-check · 0 19 * * 0"]
    skillpacks["skillpacks · 0 6 * * 0"]
    stale-content-pr-sweeper["stale-content-pr-sweeper · 45 23 * * *"]
    star-momentum-alert["star-momentum-alert"]
    startup-idea["startup-idea"]
    suggest-edges["suggest-edges · 30 5 * * *"]
    swarm-safety-eval["swarm-safety-eval · 30 7 * * 0"]
    ...
    weekly-shiplog["weekly-shiplog · 0 9 * * 1"]
  end
  subgraph external["cross-category"]
    builder-map["builder-map"]
    deploy-prototype["deploy-prototype"]
  ...
  notegraph -..->|"notegraph.json"| stale-content-pr-sweeper
  ...
  class action-converter,autoresearch-goal,contributor-spotlight,daily-routine,deal-flow,disclosure-tracker,evening-recap,fleet-state,fork-contributor-leaderboard,fork-release-tracker,fork-skill-digest,idea-capture,idea-pipeline,idea-validator,launch-radar,morning-brief,note-taking,onboard,operator-scorecard,pvr-triage-monitor,pvr-watchlist,reg-monitor,rss-feed,self-improve,signal-verdict,skill-leaderboard,star-momentum-alert,startup-idea,tool-builder,topic-momentum,update-gallery,v4-readiness,vuln-tracker,weekly-review disabled
  class vuln-scanner,builder-map,deploy-prototype,fork-cohort,market-context-refresh,vercel-projects external
...
flowchart LR
  subgraph other["other (7)"]
    compute-futures-eda["compute-futures-eda · 0 6 * * *"]
    compute-macro-correlate["compute-macro-correlate · 30 6 * * 0"]
    compute-pulse["compute-pulse · 0 11 * * 6"]
    frontier-eval["frontier-eval"]
    programmatic-eda["programmatic-eda"]
    runpod-spot-pricing["runpod-spot-pricing"]
    surplus-pulse["surplus-pulse · 30 16 * * *"]
  end
  compute-pulse -..->|"compute-pulse.md"| runpod-spot-pricing
  classDef enabled fill:#fff,stroke:#000,stroke-width:2px,color:#000
  classDef disabled fill:#f5f5f5,stroke:#bbb,color:#888
  classDef external fill:none,stroke:#bbb,stroke-dasharray:3 3,color:#888
  class compute-futures-eda,compute-macro-correlate,compute-pulse,surplus-pulse enabled
  class frontier-eval,programmatic-eda,runpod-spot-pricing disabled
...
- `-->` — declarative `depends_on` from SKILL.md frontmatter
- `-.->` — chain `consume:` (currently 0 — all chain blocks in aeon.yml are commented out)
- `-..->` — reactive trigger *or* derived shared-state edge (writer → reader through a `memory/topics/*` or `memory/state/*` file)
- **Bold border** — `enabled: true` in aeon.yml (label shows cron schedule)
- **Faded grey** — `enabled: false`
- **Dashed ghost** — cross-category dependency shown for context; not owned by this diagram
- Every skill writes `memory/cron-state.json` — collapsed here to a single self-healing-loop edge rather than N fan-in edges.
- **Click any node** — opens the skill's `SKILL.md` in the repo.
...
| Enabled | 43 |
| Disabled | 129 |
| Research & Content | 24 (enabled: 1) |
| Dev & Code | 41 (enabled: 11) |
| Crypto & Markets | 24 (enabled: 0) |
| Social | 15 (enabled: 0) |
| Productivity & Meta | 61 (enabled: 27) |
| Other | 7 (enabled: 4) |

| Edge type | Count |
|---|---|
| depends_on | 5 |
| consume (from chains) | 0 |
| reactive | 0 |
| shared-state (derived) | 27 |

---

*skills parsed: 172 · depends_on: 5 · consume: 0 · reactive: 0 · shared-state derived: 27 · enabled: 43/172 · mode: SKILL_GRAPH_NEW*
