...
// ── atomicity / bundle scoring ─────────────────────────────────────────────
// Atomic notes (memory/notes/*.md, excluding daily/) should state one claim
// in <=3 sentences. A bundled note becomes a "linking hazard" — [[X]] ends
// up pointing at a topic dump instead of a single claim. This scorer flags
// candidates for reflect/note-taking to split. MOCs (memory/topics/),
// articles (docs/), and daily indexes (memory/notes/daily/) are exempt:
// they are expected to bundle.
const ATOMIC_NOTE_PATH_RE = /^memory\/notes\/(?!daily\/)[^/]+\.md$/;

function isAtomicNotePath(rel) {
  return ATOMIC_NOTE_PATH_RE.test(rel);
}

function scoreBundling(body) {
  // Strip frontmatter, fenced code, the H1 title line, and HTML comments
  // before scoring — only the prose body counts.
  let prose = body
    .replace(/^---\n[\s\S]*?\n---\n?/, "")
    .replace(/```[\s\S]*?```/g, "")
    .replace(/<!--[\s\S]*?-->/g, "")
  ...
  // Sentence count: split on . / ! / ? followed by whitespace or EOL. Cheap
  // and noisy but consistent.
  const sentences = prose.split(/[.!?](?:\s+|$)/).filter((s) => s.trim().length > 0);
  const sentenceCount = sentences.length;

  // Connectives that almost always introduce a second claim. Case-insensitive,
  // word-bounded so "additional" / "Andover" don't trigger.
  const connectiveRe = /\b(?:and also|additionally|moreover|furthermore)\b/gi;
  const connectiveHits = (prose.match(connectiveRe) || []).length;

  // H2/H3 inside an atomic note is a strong bundling signal — atomic notes
  // shouldn't need subheadings.
  const subheadingCount = (prose.match(/^##{1,2}\s+/gm) || []).length;

  // Score: weighted sum. Threshold 3 chosen so a clean 3-sentence note with
  // no connectives and no subheadings scores 0; a 5-sentence note scores
  // exactly 3 (one tick over); a single 'and also' scores 2; a subheading
  // scores 3 by itself.
  const sentencePenalty = Math.max(0, sentenceCount - 3);
  const score = sentencePenalty + 2 * connectiveHits + 3 * subheadingCount;
...
// Dampen the raw bundle score by inDegree: a note that's earning references
// is functioning as a coherent reference even if its body is dense. Subtract
// up to 3 points based on how many other notes link in. Threshold remains 3.
// Effect: a 5-sentence note (raw=2) stays clean regardless; a 7-sentence note
// (raw=4) is flagged unless 1+ other notes reference it; a hub like
// runner-allocation-outage-2026-05-30 (inDeg 10, raw=3) drops to 0.
function dampenedBundleScore(rawScore, inDegree) {
  return Math.max(0, rawScore - Math.min(inDegree, 3));
}
...
function buildGraph() {
  ...
  const nodes = notes.map((n) => {
    const atomic = isAtomicNotePath(n.rel);
    const bundle = atomic ? scoreBundling(n.body) : null;
    const inDegree = inDeg.get(n.id) || 0;
    const bundleScore = bundle ? dampenedBundleScore(bundle.rawScore, inDegree) : null;
    return {
      id: n.id,
      title: n.title,
      dir: n.rel.split("/")[0],
      ...
      atomic,
      bundleRawScore: bundle ? bundle.rawScore : null,
      bundleScore,
      bundled: bundleScore !== null && bundleScore >= 3,
    };
  });
  ...
  return {
    ...
    stats: {
      ...
      atomic: nodes.filter((n) => n.atomic).length,
      bundled: nodes.filter((n) => n.bundled).length,
    },
    nodes,
...
function renderMermaid(graph) {
  ...
  out += `**${graph.stats.nodes} notes**, ${graph.stats.hard} hard edges, ${graph.stats.soft} soft edges, ${graph.stats.orphans} orphans, ${graph.stats.bundled} bundled atomic notes.\n\n`;

  if (graph.stats.bundled > 0) {
    const bundled = graph.nodes
      .filter((n) => n.bundled)
      .sort((a, b) => b.bundleScore - a.bundleScore);
    out += `> ⚠ **Bundled atomic notes** (dampened score ≥3 — candidates for the \`reflect\` atomicity pass to split). Score is \`max(0, rawScore - min(inDegree, 3))\` — notes earning references are not flagged.\n>\n`;
    for (const n of bundled) {
      out += `> - \`${n.id}\` — score ${n.bundleScore} (raw ${n.bundleRawScore}, inDeg ${n.inDegree}) — "${n.title}"\n`;
    }
    out += `\n`;
  }
...
function main() {
  ...
  const s = graph.stats;
  console.log(`notegraph: ${s.nodes} nodes · ${s.hard} hard · ${s.soft} soft · ${s.orphans} orphans · ${s.bundled} bundled`);
