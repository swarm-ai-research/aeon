...
// 17. gh pr list returns unparseable (malformed) JSON → catch block in
//     openPRsForKind returns null → gate is skipped, pass proceeds normally.
//     This is distinct from test 11 (gh exits non-zero) and test 12 (non-array
//     JSON): here the catch branch is the path taken.
{
  const env = freshRepoWithRemote({
    claudeScript: CLAUDE_EDIT_STUB,
  ...
  console.log("OK  gh pr list returns malformed JSON → catch branch → null backlog → pass proceeds");
}

// 18. buildPrompt no-target branch — when `target` is omitted, the prompt must
//     contain "No specific files supplied" rather than "Recently changed files".
//     This is the else-branch of the `target ?` ternary in buildPrompt().
//     Also verifies that "Additional focus" is absent when no focus is given.
{
  ...
  try {
    const result = withStubbedEnv(env.binDir, env.ghLog, () =>
      ...
      // target and focus both omitted
    );
    assert.equal(result.ok, true);
    assert.equal(result.prUrl, null, "no-op run should return prUrl:null");
    const captured = readFileSync(stdinCapturePath, "utf8");
    assert.match(captured, /No specific files supplied/,
      "prompt must use 'No specific files supplied' when target is absent");
    assert.doesNotMatch(captured, /Recently changed files/,
      "prompt must not mention 'Recently changed files' when target is absent");
    assert.doesNotMatch(captured, /Additional focus from task payload/,
      "prompt must not include focus line when focus is absent");
  } finally {
    if (oldCapture === undefined) delete process.env.CLAUDE_STDIN_CAPTURE;
    else process.env.CLAUDE_STDIN_CAPTURE = oldCapture;
    rmSync(env.work, { recursive: true, force: true });
  ...
  console.log("OK  no-target prompt branch uses 'No specific files supplied'; no focus line when focus absent");
}
