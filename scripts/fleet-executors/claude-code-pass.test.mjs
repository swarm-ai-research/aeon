...
// 15. openPRsForKind catch branch — `gh pr list` returns valid exit-0 but
//     malformed JSON that throws in JSON.parse(). The catch returns null, the
//     gate is skipped, and the pass proceeds (null = "unknown", not "at limit").
{
  const env = freshRepoWithRemote({
    claudeScript: CLAUDE_EDIT_STUB,
    ghScript: `#!/usr/bin/env bash\necho "gh: $@" >> "$GH_LOG"\nif [ "$1" = "pr" ] && [ "$2" = "list" ]; then echo 'bad{json'; exit 0; fi\necho "https://github.com/stub/repo/pull/55"\n`,
  });
  try {
    const result = withStubbedEnv(env.binDir, env.ghLog, () =>
...
// 16. buildPrompt no-target branch — when `target` is omitted the prompt
//     contains "No specific files supplied" rather than "Recently changed files".
{
  const env = freshRepoWithRemote({
    claudeScript: `#!/usr/bin/env bash\nif [ "$1" = "--version" ]; then echo "stub claude"; exit 0; fi\ncat > "$CLAUDE_STDIN_CAPTURE"\necho "nothing to change"\n`,
    ghScript: `#!/usr/bin/env bash\nif [ "$1" = "pr" ] && [ "$2" = "list" ]; then echo "[]"; exit 0; fi\necho "https://github.com/stub/repo/pull/77"\n`,
  });
  const stdinCapturePath = join(env.work, "stdin.txt");
  const oldCapture = process.env.CLAUDE_STDIN_CAPTURE;
  process.env.CLAUDE_STDIN_CAPTURE = stdinCapturePath;
  try {
    const result = withStubbedEnv(env.binDir, env.ghLog, () =>
  ...
  console.log("OK  no-target branch — prompt contains 'No specific files supplied'");
}

// 17. prUrl extraction fallback — when `gh pr create` stdout contains no URL
//     pattern the raw stdout.trim() is used as prUrl.
{
  const env = freshRepoWithRemote({
    claudeScript: CLAUDE_EDIT_STUB,
    ghScript: `#!/usr/bin/env bash\necho "gh: $@" >> "$GH_LOG"\nif [ "$1" = "pr" ] && [ "$2" = "list" ]; then echo "[]"; exit 0; fi\necho "PR opened: #99"\n`,
  ...
  try {
    const result = withStubbedEnv(env.binDir, env.ghLog, () =>
      runCodePass({ kind: "docs-pass", target: "lib.mjs", repoDir: env.localDir, taskId: "urlfallback" })
    );
    assert.equal(result.ok, true, `expected ok=true, got ${JSON.stringify(result)}`);
    assert.equal(result.prUrl, "PR opened: #99", `expected raw stdout as prUrl, got ${result.prUrl}`);
  } finally {
    rmSync(env.work, { recursive: true, force: true });
  }
  console.log("OK  prUrl extraction fallback — non-URL gh output used as prUrl verbatim");
}
