...
**Resolution (applied):** static binaries for `zizmor` and `actionlint` are now committed to `.audit-bin/` in the repo root. The skill's step 0b exports `$PWD/.audit-bin` to PATH before attempting any network install, so the pre-cached versions resolve first. The pip/curl paths remain as fallbacks for local runs where the binary may be absent or stale.
