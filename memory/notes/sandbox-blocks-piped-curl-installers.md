...
**Resolution (2026-07-15):** `.audit-bin/` was added to the repo root with statically-linked `actionlint` and `zizmor` binaries. The `workflow-security-audit` skill's Sandbox note now instructs Claude to `export PATH="$PWD/.audit-bin:$PATH"` before attempting any network-based install, eliminating the degraded-mode fallback.
