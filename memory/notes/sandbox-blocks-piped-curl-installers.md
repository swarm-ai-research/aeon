...
**Resolution (2026-07-13):** Pre-built `zizmor` and `actionlint` binaries are now committed under `.audit-bin/`. The skill's step 0b prepends `$PWD/.audit-bin` to `$PATH` before attempting any pip/curl fallback, making the scanners available in CI without network access. The SKILL.md sandbox note has been updated to document this as the primary supply path.
