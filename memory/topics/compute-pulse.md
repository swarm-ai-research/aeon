...
Current prices ($/1M tokens in/out) as of 2026-08-08 (stable week — no >10% cuts; see Pricing Signal Log):
...
*Structural signal: 1,000× aggregate cost collapse since late 2022. GPT-5.6 Luna −80% (Jul 30) matches o3 as sharpest single-tier pricing move to date — repriced <3 weeks after launch, driven by model-assisted inference optimization + open-model competition. Vera Rubin 10× tokens/Watt in full production (H2 2026 cloud deployments live) is the supply-side deflationary signal. Inference market >$50B in 2026, growing faster than training for first time. OpenAI 20% internal cost reduction via SW optimization (Jul 29 engineering post, not yet passed through as list-price cut — watch). Paradox holds: per-token prices fell 280× in 2 years while enterprise AI spend rose 320%. Memory scarcity emerging as next chokepoint. Gartner: 90%+ further drop by 2030.*

## Decentralized Compute Tokens

Prices as of 2026-08-08:

| Symbol | Project | Price | Signal |
|--------|---------|-------|--------|
| RENDER | Render Network | ~$1.37 | −1% vs Aug 1; demand>supply inflection still unpriced; network steady |
| TAO | Bittensor | ~$193 | flat vs Aug 1; holding $190 support; SEC ETF window still open — next binary catalyst |
| IO | io.net | ~$0.15 | −12% vs Aug 1; downtrend continues; Jul unlock still digesting |
| AKT | Akash Network | ~$0.50 | **−25% vs Aug 1** — notable slide; Mainnet 18 upgrade gains not holding bid |

*DePIN narrative: AKT −25% is the week's notable mover — largest single-week DePIN decline since IO's Jul unlock digestion began; Mainnet 18 usage improvements not translating to bid. IO downtrend continues (−12%). RENDER and TAO holding near Aug 1 levels. Centralized capex mega-deals (AMD 2 GW, Meta Prometheus 1 GW, Anthropic 3.5 GW Google/Broadcom) still dominating market perception. TAO SEC ETF window remains the sector's key binary catalyst. Market bifurcation intensifying: frontier API tier up 36.4% YoY while mid-tier fell 35.8% (Axis Intelligence, Jul 24). No DePIN token outperforming — centralized moat winning in market perception.*
