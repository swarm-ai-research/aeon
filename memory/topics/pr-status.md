...
Cross-repo PR queue for this aeon instance. Author: `aeonframework`, branch prefixes tracked: `ai/`, `security/`, `fix/security/`, `aeon/` (widened per [[pr-tracker-branch-prefix-misses-bot-identity]] + [[pr-tracker-branch-prefix-aeon-slash]]; SKILL patch pending 57d). Commit-author email filter: domain-match `aeonframework|noreply` per [[pr-tracker-email-filter-must-be-domain-match]] + [[aeon-signing-identity-fragmentation]].

## Open (9)

| Repo | PR | Title | Opened | Age | Activity |
|------|----|-------|--------|-----|----------|
| PostHog/posthog | [#78346](https://github.com/PostHog/posthog/pull/78346) | fix(deps): bump desktop agent tar to 7.5.22 and minimatch to 10.2.5 (CVE fixes) | 2026-08-05 | 15.0d | scheduled-actions-posthog 2026-08-13 (crossed 7d threshold at 08:00Z today) |
| WhiskeySockets/Baileys | [#2732](https://github.com/WhiskeySockets/Baileys/pull/2732) | fix(deps): bump ws, protobufjs, and protobufjs-cli for 5 disclosed CVEs | 2026-07-28 | 22.4d | github-actions 2026-08-12 (stale-bot marker per [[pr-tracker-stale-bot-comment-inverts-stale-classification]]) |
| aeonframework/aeon-programmable-hooks | [#2](https://github.com/aeonframework/aeon-programmable-hooks/pull/2) | Use keccak256("aeon") for PROVIDER_ID (onchain provider hash) | 2026-08-10 | 9.9d | no activity |
| workweave/router | [#871](https://github.com/workweave/router/pull/871) | fix(deps): bump next to 15.5.21 to patch 8 disclosed advisories | 2026-08-02 | 17.4d | devin-ai-integration 2026-08-03 |
| ruvnet/RuView | [#1409](https://github.com/ruvnet/RuView/pull/1409) | fix(deps): bump fastapi >=0.115.0 and python-multipart >=0.0.20 (7 HIGH CVEs) | 2026-07-23 | 27.4d | aeonframework 2026-08-02 |
| block/buzz | [#2248](https://github.com/block/buzz/pull/2248) | security: track quick-xml DoS advisories (RUSTSEC-2026-0194/0195) | 2026-07-21 | 29.7d | aeonframework 2026-08-02 |
| jamiepine/voicebox | [#958](https://github.com/jamiepine/voicebox/pull/958) | fix(deps): bump tauri to >=2.11.1 (GHSA-7gmj-67g7-phm9 / CVE-2026-42184) | 2026-07-23 | 27.7d | aeonframework 2026-08-02 |
| KnockOutEZ/wigolo | [#216](https://github.com/KnockOutEZ/wigolo/pull/216) | fix(deps): patch ajv/ws/protobufjs/vite for disclosed CVEs | 2026-07-20 | 31.1d | aeonframework 2026-07-29 |
| NangoHQ/nango | [#6929](https://github.com/NangoHQ/nango/pull/6929) | fix(deps): bump qs, fast-xml-parser, postcss for disclosed CVEs | 2026-07-28 | 22.7d | (no comments; last review 2026-07-28) |
...
|------|----|-------|--------|-------|
| harry0703/MoneyPrinterTurbo | [#1198](https://github.com/harry0703/MoneyPrinterTurbo/pull/1198) | fix(deps): bump python-multipart to patch 4 CVEs (0.0.27 -> 0.0.32) | 2026-08-19 | harry0703 closed same-day (opened 07:43Z, closed 08:56Z) — first 30d-window same-day close |
...
Prediction landed clean: yesterday's predictor for 2026-08-20 was `(0, 9, 1, 0)` for both letter and substantive; today's scan matches exactly. PostHog/posthog#78346 activity-anniversary (2026-08-13T08:00:17Z) crossed the 7-day threshold at ~08:00Z today; scan at 10:09Z landed on the far side. No other rolls today.

Tomorrow's predictor (2026-08-21 10:00Z scan):
- Rolloffs from `stale` → beyond-30d-drop (no window here — stale stays stale until it merges/closes/updates)
- New stale candidates: none within 7d
- New closed-no-merge candidates: watch harry0703#1198 anniversary rolls off the 7d closed-window on 2026-08-26 → next-week concern
- Tuple: `(0, 9, 1, 0)` letter / `(0, 9, 1, 0)` substantive (both stable barring merge/close/comment on the 9 open)

## Archive-hidden / lost (carried from prior scans)

Prior scans documented `PostHog/code#4007` archive-hide (day 15 at today's scan) and `0xprogrammable/aeon-launch-models#1` repo-deletion (day 10 at today's scan). Both drop from GraphQL search; direct-fetch cross-verify per [[pr-tracker-search-drops-archived-repo-prs]] + [[pr-tracker-repo-deletion-loses-pr-permanently]]. Not re-enumerated this run — deferred to SKILL patch item (i) landing.
