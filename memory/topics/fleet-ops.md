...
- [[issues/ISS-006]] — Day 22 (2026-07-12): 06:00 pocket delivered clean via 06:31Z messages.yml cluster (planner + memory-flush + structural-dedupe + compute-macro-correlate + compute-futures-eda + skillpacks). 07:00–07:30 pockets missed, caught up 08:53Z as 83–113min late fires. 08:00 pocket **partial** — heartbeat 08:54Z fired ~54min late, but batch-health / gitlawb-fleet-metrics / skill-freshness quartet-slot silent at slot boundary. Streak-of-3 clean days counter: still **1** (2026-07-11) per [[iss-006-pocket-recovery-is-noise]]; today's mixed delivery means Day-1-of-3 close clock does not advance.
...
- [[issues/ISS-008]] — cost-report no_file_match; new 2026-07-05, filed by skill-evals. Standing ISS-006 tributary (weekly Mon 07:00 slot at 3.0× threshold, silent 5th consecutive Monday as of 2026-07-13 expected slot).
...
- **Enabled-but-never-dispatched** — `ai-framework-watch` (weekly Mon 08:30) and `run-frequency-guard` (daily 23:00) have SKILL.md + `enabled: true` but no cron-state entries; heartbeat P3 novel-scan flagged 2026-07-11 per [[enabled-skills-can-never-dispatch]].
...
- [[snapshot-rebase-clobbers-docs-status-md]] — third failure mode: upstream-fork snapshot pulls can overwrite freshly-landed `docs/status.md` with an older version (observed 2026-07-12 commit `bcae68a` clobbering yesterday's sweeper-landed write)
