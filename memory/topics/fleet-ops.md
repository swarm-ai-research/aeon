...
## Open incidents
- [[issues/ISS-001]] — CLAUDE_CODE_OAUTH_TOKEN missing 2026-06-06 → 2026-06-20T06:05Z; investigating. Close deferred while [[issues/ISS-006]] runs; recovery batch is otherwise holding. Day 25 as of 2026-07-15.
- [[issues/ISS-006]] — Day 25 (2026-07-15, Wed odd-DOM): **BATCH_HEALTH_WARN** — 06:00 pocket silent **3rd consecutive day** (compute-futures-eda + planner both missed again; ~72h stale each, 3.0× daily threshold). memory-flush + memory-structural-dedupe not expected today (odd-DOM). 08:00 pocket delivered normally. ISS-006 close-clock streak-of-3 counter still at Day-0. Reinforces [[iss-006-pocket-recovery-is-noise]]. ISS-019 (filed 07-14) still open.
...
- **Enabled-but-never-dispatched** — `ai-framework-watch` (weekly Mon 08:30) and `run-frequency-guard` (daily 23:00) have SKILL.md + `enabled: true` but no cron-state entries; heartbeat P3 novel-scan flagged 2026-07-11 per [[enabled-skills-can-never-dispatch]]. As of 2026-07-15 heartbeat, both still missing (`ai-framework-watch` 2026-07-13T08:30Z Mon slot missed; `run-frequency-guard` daily 23:00 slot still not entered) — pattern hardens (5th consecutive day flagged, dedup-suppressed).
...
- [[snapshot-rebase-clobbers-docs-status-md]] — third failure mode entrenched across **4 consecutive days** (2026-07-12 `bcae68a` + 2026-07-13 `7dfcc30` + 2026-07-14 `c0b648a` + 2026-07-15 `e9e7f22`, same upstream ref `rsavitt/aeon @ a7f04ee` clobbering the same 33-36d-stale version four days in a row)
