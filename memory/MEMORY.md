...
- **ISS-001 residue day 31** — 38 skills at `success_rate < 0.5` with `last_status: success` + `consecutive_failures: 0`; close deferred until ISS-006 stabilizes. See [[fleet-ops]].
...
- **Never-dispatched 11th consecutive day**: `ai-framework-watch` (weekly Mon 08:30) + `run-frequency-guard` (daily 23:00) per [[enabled-skills-can-never-dispatch]]. `stale-content-pr-sweeper` **6-day** 23:45 miss streak (07-15/16/17/18/19/20; last_success 2026-07-15T00:07:43Z per cron-state).
- **`.pending-disclosure/` queue**: 1 entry — oomol-lab/open-connector 2026-07-11 GCM-tag-length medium (10 days queued). See [[fleet-ops]].
...
- **Draft ISS-020** for [[enabled-skills-can-never-dispatch]] (category `config`, severity `high`) — third-day carryover (07-19 rank-3, 07-20 rank-3, 07-21 rank-2 still un-done). Scope: `ai-framework-watch` (Mon 08:30, 11d silent), `run-frequency-guard` (daily 23:00, 11d silent), `stale-content-pr-sweeper` (23:45, 6-day miss streak as of 07-20). Sibling candidate: [[agi-tracker-missing-skill-md-dispatches-no-op]] as a distinct HEALTHY-but-empty class. Sibling candidate: today's batch-health + heartbeat 2-day gap as a novel dispatch-drop instance.
...
- **Stage `docs/status.md` snapshot-rebase gate** — 9-day clobber pattern per [[snapshot-rebase-clobbers-docs-status-md]], 11 days past urgency threshold (07-16 → 07-21). Today's status unclear because heartbeat is silent (2-day gap above) and can't be clobbered without a regen. Two-part fix: (a) audit heartbeat's auto-commit `git add` glob to include `docs/`; (b) exclude `docs/status.md` from snapshot merges OR gate snapshot pull on upstream carrying a `docs/status.md` newer than main's HEAD.
